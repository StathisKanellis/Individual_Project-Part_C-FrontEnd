...
function readForm(){
    var formData = {};
    formData["trainer"]=document.getElementById("trainer").value;
...
function insertRecord(data){
    var table=document.getElementById("trainerPerCourse").getElementsByTagName('tbody')[0];
    var newRow=table.insertRow(table.length);
    cell1=newRow.insertCell(0);
    cell1.innerHTML=data.trainer;
...
function resetForm(){
    document.getElementsById("trainer").this.reset();
...
function onEdit(td){
    ...
    document.getElementsById("trainer").value=selectedRow.cells[0].innerHTML;
...
function upDate(formData){
    selectedRow.cells[0].innerHTML=formData.trainer;
